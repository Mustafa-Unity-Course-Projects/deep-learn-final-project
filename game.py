...
worker_sprite = pygame.image.load("worker.png")
worker_sprite = pygame.transform.scale(worker_sprite, (300, 300))
...
while is_running:
    ...
    decision = model(list_to_tensor(factoryState.to_list())).max(1).indices.view(1,1)[0][0]
    lights_on = decision % 2
    ac_on = decision // 2

    # Draw the floor (grass).
    pygame.draw.rect(window_surface, pygame.Color(0, 200, 0), [0, 500, 1200, 100])
    # Draw the building
    pygame.draw.rect(window_surface, pygame.Color(128, 128, 128), [400, 100, 700, 400], 20)
    ...
    if factoryState.employees_exist:
        window_surface.blit(worker_sprite, (500, 200))
